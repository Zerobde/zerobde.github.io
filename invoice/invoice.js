computed: {
  allItems() {
    return []
      .concat(Array.isArray(this.invoice.Items) ? this.invoice.Items : [])
      .concat(Array.isArray(this.invoice.Items2) ? this.invoice.Items2 : [])
      .concat(Array.isArray(this.invoice.Items3) ? this.invoice.Items3 : []);
  },

  calculatedSubtotal() {
    return this.allItems.reduce((sum, item) => {
  ...
  calculatedTotal() {
    return (
      ...
      - (Number(this.invoice.Deduction) || 0)
...
/**
 * 
 * All the above is new per ChatGPT 
 */
...
if (!row.Inv_Supplies) {
    row.Inv_Supplies = [
      {
        Description: 'Inv_Supplies[0].Description',
        Quantity: '.Quantity',
        Total: '.Total',
        Price: '.Price',
      },
      {
        Description: 'Inv_Supplies[1].Description',
        Quantity: '.Quantity',
        Total: '.Total',
        Price: '.Price',
      },
...
if (!row.Inv_Decorations) {
    row.Inv_Decorations = [
      {
        Description: 'Inv_Decorations[0].Description',
        Quantity: '.Quantity',
        Total: '.Total',
        Price: '.Price',
      },
      {
        Description: 'Inv_Decorations[1].Description',
        Quantity: '.Quantity',
        Total: '.Total',
        Price: '.Price',
      },
...
if (!row.Inv_Labor) {
    row.Inv_Labor = [
      {
        Description: 'Inv_Labor[0].Description',
        Quantity: '.Quantity',
        Total: '.Total',
        Price: '.Price',
      },
      {
        Description: 'Inv_Labor[1].Description',
        Quantity: '.Quantity',
        Total: '.Total',
        Price: '.Price',
      },
...
function updateInvoice(row) {
  try {
    ...
    const importance = ['Number', 'Client', 'Items', 'Items2', 'Total', 'Invoicer', 'Due', 
